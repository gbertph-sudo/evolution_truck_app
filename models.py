...
from sqlalchemy import (
    ...
    Text, Numeric, CheckConstraint, Index, UniqueConstraint, SmallInteger,
    Table, Column
)
...
# (SIN CAMBIOS, lo dejo igual que lo tenías)
# ======================================================
class Supplier(Base):
...
class InventoryItemImage(Base):
    ...
    __table_args__ = (Index("ix_item_images_item_pos", "item_id", "position"),)
...
class LegacyInventoryItem(Base):
    ...
    movements: Mapped[List["LegacyStockMovement"]] = relationship(
        "LegacyStockMovement", back_populates="item", cascade="all, delete-orphan"
    )


class LegacyStockMovement(Base):
    __tablename__ = "stock_movements_legacy"
