const ACCOUNTING_LEDGER_CONFIG = {
  key: "accounting_ledger",
  title: "Accounting / Ledger",
  apiPrefix: "/accounting-ledger"
};

async function loadACCOUNTING_LEDGERMeta() {
  const statusBox = document.getElementById("module-status");
  if (!statusBox) return;

  statusBox.textContent = "Loading...";
  try {
    const response = await fetch(`${ACCOUNTING_LEDGER_CONFIG.apiPrefix}/meta`);
    if (!response.ok) {
    ...
    statusBox.innerHTML = `
      ...
      <strong>Module:</strong> Accounting / Ledger<br>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btn) {
    btn.addEventListener("click", loadACCOUNTING_LEDGERMeta);
  }
  loadACCOUNTING_LEDGERMeta();
});
