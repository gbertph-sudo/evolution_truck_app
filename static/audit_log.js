...
async function loadAUDIT_LOGMeta() {
  const statusBox = document.getElementById("module-status");
  if (!statusBox) return;

  statusBox.textContent = "Loading...";
  try {
    const response = await fetch(`${AUDIT_LOG_CONFIG.apiPrefix}/meta`);
    if (!response.ok) {
    ...
    statusBox.innerHTML = `
      ...
      <strong>Module:</strong> Audit Log<br>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btn) {
    btn.addEventListener("click", loadAUDIT_LOGMeta);
  }
  loadAUDIT_LOGMeta();
});
